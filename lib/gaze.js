...
Gaze.prototype.processPatterns = function(patterns, processFn, done) {
  var _this = this;
  var result = [];
  var patterns = _.flatten(patterns);
  forEachSeries(patterns, function(pattern, next) {
    processFn(pattern, function(err, matches) {
      ...
      if (exclusion) {
        ...
        if (result.length < 25) {
          ...
          console.log('Result omitted, too large');
        }*/
   ...
   /* console.log('************FINAL*********** PROCESSPATTERN');
...
Gaze.prototype.add = function(patterns, done) {
  ...
  _this.processPatterns(patterns, function(pattern, done) {
