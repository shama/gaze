...
Gaze.prototype.add = function(files, done) {
  ...
  helper.forEachSeries(files, function(file, next) {
    self._platform(path.join(self.options.cwd, file));
    next();
  }, function() {
...
// Wrapper for adding files to the platform
Gaze.prototype._platform = function(file) {
  try {
    platform(file, this._trigger.bind(this));
  } catch (err) {
    this.emit('error', err);
  }
};
...
Gaze.prototype._wasAddedSub = function(dir) {
  ...
  fs.readdir(dir, function(err, current) {
    helper.forEachSeries(current, function(file, next) {
      ...
      } catch (err) { /* ignore error if ENOENT */ }
      next();
    });
  });
};
