...
Gaze.prototype.add = function (globs, done) {
  if (typeof files === 'string') { globs = [globs]; }
  this._patterns = helper.unique.apply(null, [this._patterns, globs]);
  var files = globule.find(this._patterns, this.options);
  if (files.length == 0) {
    files.push(this.options.cwd); 
  }
