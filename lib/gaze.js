...
Gaze.prototype.emit = function() {
  ...
  if (e === 'added') {
    ...
    var relativepath = path.relative(self.options.cwd, filepath);
    if (!globule.isMatch(self._patterns, relativepath, self.options)) {
      return false;
    }
  } else {
...
Gaze.prototype.add = function(files, done) {
  ...
  //console.log('files', this._patterns, this.options, files)
    ...
    try {
      ...
      platform(filepath, self._trigger.bind(self), function(watcher) {
        ...
        if (!helper.isDir(filepath)) {
          ...
          platform(path.dirname(filepath), self._trigger.bind(self), function(watcher) {
            self._watchers[filepath] = watcher;
          });
        }
      });
...
Gaze.prototype._wasAdded = function(dir) {
  ...
  fs.readdir(dir, function(err, current) {
    ...
    helper.forEachSeries(current, function(file, next) {
      ...
      if (self._watchers[filepath]) return next();
      var stat;
      try {
...
Gaze.prototype.relative = function(dir, unixify, done) {
  ...
  //console.log('result', Object.keys(this._watchers))
