...
Gaze.prototype.emit = function() {
  ...
  if (cache.indexOf(e) === -1) {
    _objectPush(self._cached, filepath, e);
...
Gaze.prototype.close = function(_reset) {
  ...
  Object.keys(this._watched).forEach(function(dir) {
    fs.unwatchFile(dir);
    self._watched[dir].forEach(function(uFile) {
...
Gaze.prototype.add = function(files, done) {
  ...
  files = globule.find(this._patterns, self.options);
  self._addToWatched(files);
  self.close(false);
  self._initWatched(done);
};
...
Gaze.prototype._addToWatched = function(files) {
  ...
  files.forEach(function(file) {
    var filepath = path.resolve(self.options.cwd, file);
    if (file.slice(-1) === '/') { filepath += path.sep; }
    _objectPush(self._watched, path.dirname(filepath) + path.sep, filepath);
  });
  return this;
};
...
Gaze.prototype._watchDir = function(dir, done) {
  var self = this;
  try {
    self._watchers[dir] = fs.watch(dir, function(event) {
...
Gaze.prototype._unpollDir = function(dir) {
  ...
  this._watched[dir].forEach(function(file) {
    _this._unpollFile(file);
  });
};

Gaze.prototype._pollFile = function(file, done) {
    var self = this;
    var opts = { persistent: true, interval: self.options.interval };
    try {
    ...
    self._watchDir(dir, function(event, dirpath) {
      ...
      fs.readdir(dirpath, function(err, current) {
        ...
        current.filter(function(file) {
          ...
          if (self._isMatch(relFile)) {
            // Add to watch then emit event
            self.add(relFile, function() {
