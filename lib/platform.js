...
function cbstack(err, event, filepath, newFile) {
  if (watched[filepath]) {
  ...
  if (platform.mode === 'auto' || platform.mode === 'watch') {
    ...
    setTimeout(function() {
      ...
      if (process.platform === 'linux' && renameWaiting) {
        ...
        cbstack(null, 'rename', renameWaitingFile, file);
      ...
      try {
        watched[file].push(PathWatcher.watch(file, function(event, newFile) {
          ...
          cbstack(null, event, filepath, newFile);
        }));
      } catch (error) {
